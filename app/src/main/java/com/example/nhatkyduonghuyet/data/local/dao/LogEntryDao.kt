...
interface LogEntryDao {
    ...
    /** One-shot snapshot for the backup pipeline. */
    @Query("SELECT * FROM log_entries ORDER BY date DESC, time DESC")
    suspend fun getAllLogEntriesOnce(): List<LogEntry>

    /**
     * Upsert used when restoring: an entry is identified by (date, session)
     * rather than by its autogenerated id, so re-importing the same file twice
     * updates the row instead of duplicating it.
     */
    @Query("SELECT * FROM log_entries WHERE date = :date AND session = :session LIMIT 1")
    suspend fun findByDateAndSession(date: String, session: String): LogEntry?
